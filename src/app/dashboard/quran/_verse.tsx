...
export default function VerseComponent({ verse, isLink }: { verse: Database['public']['Tables']['DataQuran']['Row'], isLink?: boolean }) {
    ...
    const wordByWordData = Data.DataQuranWordByWord.filter(i => i.verse_id === verse.verse_id);
    ...
    const renderHighlightedText = (text?: string | null) => {
        ...
        return text.split(/(\*\*.*?\*\*)/g).map((part, index) =>
            part.startsWith("**") && part.endsWith("**") ? (
                <span key={index} className={`bg-yellow-300 dark:bg-yellow-800 rounded`}>
                    {part.slice(2, -2)}
                </span>
            ) : (
                part
        ...
        >
            <section>
                <div className="flex flex-col">
                        ...
                        {isLink && (
                            <Badge variant="secondary" className="mb-2 text-md w-fit">
                                <span className="font-light">
                                    <a href={`/dashboard/quran/reader/chapter/${verse.chapter_number}?verse=${verse.verse_number}`}>
                                        <span className="font-light">
                                            Go to verse →
                                        </span>
                                    </a>
                    ...
                    {useSettings.settings.showSubtitles && verse.verse_subtitle_english && verse.verse_subtitle_english !== "null" && (
                ...
                <div className="mx-2 flex justify-between items-start font-light">
                    <p className="w-3/6 pr-4">{renderHighlightedText(verse.verse_text_english)}</p>
                                    ...
                                    <TooltipProvider delayDuration={0} key={wordIndex}>
                                        <Tooltip>
                                            <TooltipTrigger asChild>
                                                <div>
                                                    <p className="text-xl hover:text-yellow-500">{word}</p>
                                                </div>
                                            </TooltipTrigger>
                                            <TooltipContent asChild>
                                                <Badge>
                                                    <span dir='ltr'>
                                                        {wordByWordData?.filter(i => i.verse_id === verse.verse_id).map(w => w.english_text)[wordIndex] ?? "–"}
                                                    </span>
                                                </Badge>
                                            </TooltipContent>
                                        </Tooltip>
                ...
                {useSettings.settings.showFootnotes && verse.verse_footnote_english && verse.verse_footnote_english !== "null" && (
                ...
                <Suspense fallback={<LoadingIcon />}>
                    <section>
                        <div className="flex flex-col space-y-1 md:w-3/6">
                            {wordByWordData.filter(w => w.verse_id == verse.verse_id)?.map((w, idx) => (
