...
export default function QuranChapterTitle({
  data,
}: {
  data:
    | Database["public"]["Tables"]["DataQuran"]["Row"]
    | Database["public"]["Tables"]["DataQuran"]["Row"][];
}) {
  var title = "";
  ...
  if (!data || (Array.isArray(data) && data.length === 0)) {
    ...
    if (
      data.every(
        (i) => i.chapter_title_english !== data?.[0].chapter_title_english,
      )
    ) {
          ...
          <section className="flex flex-wrap gap-2 rounded-lg">
            <Badge variant="secondary">
              <span className="font-light">
                {isMultipleChapters ? data.length : data?.[0].chapter_verses}{" "}
                verses
              </span>
            </Badge>
            {!isMultipleChapters && (
              <>
                <Badge variant="secondary">
                  <span className="font-light">
                    {data?.[0].chapter_title_arabic_transliteration}
                  </span>
                </Badge>
                <Badge variant="secondary">
                  <span className="font-light">
                    {data?.[0].chapter_title_arabic}
                  </span>
                </Badge>
