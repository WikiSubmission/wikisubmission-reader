...
export function Menu({ isOpen }: MenuProps) {
  ...
  const [verseHistoryPopoverOpen, setVerseHistoryPopoverOpen] = useState<boolean>(false);
    ...
    <ScrollArea className="[&>div>div[style]]:!block">
      <nav className="mt-8 h-full w-full">
        <ul className="flex min-h-[calc(100vh-48px-36px-16px-32px)] flex-col items-start space-y-1 px-2 lg:min-h-[calc(100vh-32px-40px-32px)]">
          ...
          <li className="flex w-full grow items-end pb-10">
            <div className="h-fit w-full bg-background/80">
              <div className="flex h-full w-full flex-col">
                <div className="flex items-center justify-between pr-4">
                  <div className="flex flex-row items-center">
                    <Button
                      variant="ghost"
                      className="m-0 flex items-center justify-center py-0"
